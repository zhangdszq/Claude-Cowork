...
agent.post('/start', async (c) => {
  const body = await c.req.json<{
    ...
    allowedTools?: string;
    prompt: string;
    externalSessionId?: string;  // ID from Electron for tracking
  }>();
...
agent.post('/continue', async (c) => {
  const body = await c.req.json<{
    sessionId: string;  // This is the claudeSessionId for resuming
    prompt: string;
    cwd?: string;
    title?: string;
    externalSessionId?: string;  // ID from Electron for tracking
  ...
  async function* eventGenerator(): AsyncGenerator<ServerEvent> {
    ...
    yield* runClaude({
      prompt: body.prompt,
      session: tempSession,
      resumeSessionId: body.sessionId,  // Use the passed claudeSessionId
      onSessionUpdate: (updates) => {
